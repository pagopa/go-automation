...
describe('scanServiceLogs fallbackUuid projection', () => {
  it('extracts the UUID from the canonical FALLBACK-UUID:<uuid> form', () => {
    ...
    assert.strictEqual(scanServiceLogs(rows, SCHEMA).fallbackUuid, '2ae8a94e-50fc-445a-b9e2-989637cc129f');
  });
  ...
  it('returns the first match across multiple rows', () => {
    ...
    assert.strictEqual(scanServiceLogs(rows, SCHEMA).fallbackUuid, '11111111-1111-1111-1111-111111111111');
  });
  ...
  it('returns undefined when no row contains the token', () => {
    ...
    assert.strictEqual(scanServiceLogs(rows, SCHEMA).fallbackUuid, undefined);
  });
  ...
  it('reads @message when message field is absent', () => {
    ...
    assert.strictEqual(scanServiceLogs(rows, SCHEMA).fallbackUuid, '33333333-3333-3333-3333-333333333333');
  });
});
