...
function applicationLogRows(messages: ReadonlyArray<string>): ReadonlyArray<ReadonlyArray<LogRowField>> {
  return messages.map((message) => [
    { field: '@timestamp', value: '2026-08-31 13:04:38.581' },
    { field: 'pod_app', value: CATALOG_READMODEL_WRITER_SQL_ALARM.podApp },
...
function context(stepResults: ReadonlyArray<readonly [string, unknown]>): RunbookContext {
  return {
    executionId: 'test',
    startedAt: new Date('2026-08-31T13:04:38.581Z'),
...
describe('INTEROP catalog readmodel writer SQL known cases', () => {
  const evaluator = new ConditionEvaluator();

  it('declares exactly the two documented cases with unique IDs and priorities', () => {
  ...
  it('matches both documented network messages, including the Confluence whitespace variant', () => {
    const knownCase = knownCaseById('catalog-readmodel-writer-temporary-network-errors');

    for (const message of DOCUMENTED_NETWORK_MESSAGES) {
      const ctx = context([
        [CATALOG_READMODEL_WRITER_SQL_ALARM.stepIds.queryApplicationLogs, applicationLogRows([message])],
      ]);
  ...
  it('matches the documented Kafka coordinator message in CID tracker evidence', () => {
    const knownCase = knownCaseById('catalog-readmodel-writer-kafka-coordinator-member-rejoin');
    const rows = applicationLogRows([DOCUMENTED_KAFKA_MESSAGE]);
    const ctx = context([[CATALOG_READMODEL_WRITER_SQL_ALARM.stepIds.queryCidTracker, [{ cid: 'cid-1', rows }]]]);
  ...
  it('does not broaden the cases to undocumented generic network or Kafka errors', () => {
    const network = knownCaseById('catalog-readmodel-writer-temporary-network-errors');
    const kafka = knownCaseById('catalog-readmodel-writer-kafka-coordinator-member-rejoin');
    const ctx = context([
      [
  ...
  it('keeps unresolved document cases open and maps downstream NA to the INTEROP census', () => {
    ...
    assert.deepStrictEqual(knownCaseById('catalog-readmodel-writer-kafka-coordinator-member-rejoin').analysis?.links, [
